...
const masterSeed = async () => {
    ...
    const filePath = path.resolve(dirname, 'media_import/hero.png')
    let heroImageId;
    
    try {
        // Check if media already exists (by filename check approximation or just creating new)
        ...
        console.log('✅ Hero Image uploaded:', heroImageId)
    } catch (e) {
        console.warn('⚠️ Media upload failed (maybe already exists or file missing). Continuing without image.', e.message)
    }
    ...
    const homeData = {
        ...
        _status: 'published',
        hero: {
            type: heroImageId ? 'highImpact' : 'mediumImpact',
            media: heroImageId ? heroImageId : undefined,
            richText: {
                root: {
                    type: 'root',
                    children: [
